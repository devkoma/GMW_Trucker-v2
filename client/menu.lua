...
RMenu.Add("GMW_Scripts:mainMenu", 'main', RageUI.CreateMenu(MenuName, "~b~ ", menuPosition["x"], menuPosition["y"], "themes", Config.Theme))
RMenu.Add("GMW_Scripts:subMenu", "subMenu", RageUI.CreateSubMenu(RMenu:Get("GMW_Scripts:mainMenu", 'main'), MenuName, "~g~Legaler Transport", menuPosition["x"], menuPosition["y"], "themes", Config.Theme))
RMenu.Add("GMW_Scripts:subMenuExport", "subMenuExport", RageUI.CreateSubMenu(RMenu:Get("GMW_Scripts:mainMenu", 'main'), MenuName, "~r~Illegaler Transport", menuPosition["x"], menuPosition["y"], "themes", Config.Theme))
RMenu.Add("GMW_Scripts:F6Menu", "F6Menu", RageUI.CreateMenu(MenuName, "~b~LS Logistics", menuPosition["x"], menuPosition["y"], "themes", Config.Theme))
...
local MenuInfos = {
    ...
    subMenuExport = RMenu:Get("GMW_Scripts:subMenuExport", "subMenuExport"),
...
function MenuInfos:RMenu()
    RageUI.IsVisible(self.mainMenu, function()
        ...
        RageUI.Separator("-")
        RageUI.Button("~g~Legale Route auswählen", nil, {RightLabel = "→→→"}, true, {
            onSelected = function() end
        }, self.subMenu)
        RageUI.Button("~r~Illegale Route auswählen", nil, { RightLabel = "→→→" }, true, {
            onSelected = function () end
        }, self.subMenuExport)
    end)
    ...
    --[[ ROUTEN_EXPORT ]]--
    RageUI.IsVisible(self.subMenuExport, function()
        for k, v in pairs(Config.Export) do 
            if has_value(MenuTable, k) then 
                ...
                if JobStarted then 
                    ...
                    RageUI.Button(v.Label, "Ware: ~b~"..v.Label, {RightLabel = v.price}, true, {
                        onSelected = function()
                            RageUI.CloseAll()
                            table.insert(MenuTable, k)
                            JobStarted = true; DriveToCoords = true; JobInfo = v; 
                            TriggerEvent("GMW_Scripts:TJ_StartJobExport", v)
                        end 
                    })
                end 
            end 
